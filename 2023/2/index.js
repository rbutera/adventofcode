...
function getMaxes(input) {
  // input will be an array of objects with shape { red: number, blue: number, green: number }
  // result will be { red: x, blue: y, greeN: z} where x y and z are the maximum values for these.
...
function splitGames(input) {
  ...
  return asGames.map((game) => {
    const id = Number.parseInt(game[0].replace("Game ", ""));
    const pulls = game[1].split(";").map((pull) => {
      const colors = pull
        .trim()
        .split(",")
        .map((entry) => {
          const [color, count] = entry.trim().split(" ");
          return [count, Number.parseInt(color)];
        });
      return {
        red: 0,
        green: 0,
...
function isPossible(input, game) {
  for (const color of ["red", "green", "blue"]) {
    if (input[color] < game.maxes[color]) {
      return false;
    }
  }
...
const games = {
  training: splitGames(training),
  input: splitGames(input),
};

function getPossibleGames(allGames, configuration) {
  const possibleGames = allGames
    .filter((game) => isPossible(configuration, game))
    .map((x) => x.id);
  console.log(possibleGames.join(", "));
